...
Root and configured-index responses call `ctx.connection.authorizeIndex` before reading HTML. In Connection's default `legacy` mode, a valid process token receives a 303 redirect plus the persistent browser cookie, an existing valid cookie serves the index, and every other index request receives the Connection-owned 401 response. In `principalMode: required`, Connection delegates the decision to the Host Principal Provider, which can serve the index, begin OIDC login, or return 401; a missing or failed provider returns 503 without reading HTML. Non-index files remain public static assets. Connection owns the selected authentication mode and its response semantics.
