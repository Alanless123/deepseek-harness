...
根路径与配置的 index 响应会在读取 HTML 前调用 `ctx.connection.authorizeIndex`。在 Connection 默认的 `legacy` 模式中，有效进程 token 会得到 303 重定向与持久浏览器 cookie，已有有效 cookie 时直接提供 index，其他 index 请求得到 Connection 所有的 401 响应。在 `principalMode: required` 中，Connection 把决定委托给 Host Principal Provider；它可以提供 index、发起 OIDC 登录或返回 401，提供方缺失或失败时则在读取 HTML 前返回 503。非 index 文件仍是公开静态资源。Connection 拥有所选认证模式及其响应语义。
