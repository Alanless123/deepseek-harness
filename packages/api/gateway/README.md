...
A stream Remote uses `@Remote({ mode: 'stream' })` and returns an `Iterable` or `AsyncIterable`. `ctx.typertGateway.stream()` applies the same endpoint, argument, lookup, and cancellation checks as unary invocation, then validates each yielded item with the generated result codec. The Client opens the Gateway-owned `/api/remote.mux` WebSocket when its plugin activates, keeps it connected while idle, and retries physical connection failures with capped backoff. A Host Principal expiry timer closes its physical generation at the exact authenticated deadline, and active-context checks guard every logical open and delivery. The first Host frame classifies the generation and carries an opaque Host-derived session binding for authenticated contexts; a physical candidate and each logical call waiting for a usable Host first frame are independently bounded to 30 seconds. After an ambiguous physical close, an authenticated generation gets a bounded 30-second recovery window: accepted logical streams retry only after a replacement Host first frame proves the same non-null binding. Calls queued after any accepted generation remember its complete classification and binding, and are rejected rather than sent if the replacement Principal or legacy/authenticated mode differs. A changed or missing binding, an explicit server policy close, or upgrade and pre-hello failures that continue until the recovery deadline terminate the old streams and bound waiters with `remote-stream-policy`; a replacement socket remains available only for calls started after its identity is accepted. Legacy generations report carrier loss immediately so their domain supervisor can reopen them, but the supervised retry must receive a legacy Host first frame within its per-call window and cannot cross into an authenticated generation. The Host sends Ping control frames at the configured `websocketHeartbeatIntervalMs` interval (30 seconds by default), and the browser answers Pong at the WebSocket protocol layer, so idle network intermediaries see traffic without any Remote stream frame. Independently cancellable logical streams share that socket; an in-process Connection carrier provides equivalent streams directly without opening it.
