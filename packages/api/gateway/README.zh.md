...
流式 Remote 使用 `@Remote({ mode: 'stream' })` 并返回 `Iterable` 或 `AsyncIterable`。`ctx.typertGateway.stream()` 执行与一元调用相同的 endpoint、参数、lookup 和取消校验，再用生成的 result codec 校验每个产出项。Client 插件激活时打开 Gateway 自有的 `/api/remote.mux` WebSocket，使其在空闲时保持连接，并以有上限的退避重试物理连接失败。Host Principal 到期计时器会在认证截止时刻关闭对应物理 generation，且每次逻辑流打开和投递都会复核上下文是否仍有效。Host 首帧会标记 generation 类型，并为已认证上下文携带由 Host 派生的不透明 session binding；物理候选连接和每个等待可用 Host 首帧的逻辑调用均有独立的 30 秒上限。发生含糊的物理关闭后，已认证 generation 进入最长 30 秒的恢复窗口：只有替代 Host 首帧证明同一个 non-null binding 后，已接受的逻辑流才可重试。任何已获准 generation 之后排队的调用都会记住其完整类型与 binding；替代 Principal 或 legacy/authenticated 模式不同时，这些调用会被拒绝而不会发出。binding 改变或缺失、服务端明确策略关闭，或者 upgrade 与首帧前失败持续到恢复截止时刻，都会让旧流及绑定的 waiter 以 `remote-stream-policy` 终止；替代 socket 只能供其身份获准后才开始的调用使用。Legacy generation 会立即报告载体断开，使领域 supervisor 可以重新打开逻辑流，但受监督的重试必须在调用上限内收到 legacy Host 首帧，且不得跨入 authenticated generation。Host 按配置的 `websocketHeartbeatIntervalMs` 间隔（默认 30 秒）发送 Ping 控制帧，浏览器在 WebSocket 协议层自动回复 Pong，使空闲网络中间层持续看到流量，而不新增 Remote stream frame。可独立取消的逻辑流共享这条连接；进程内 Connection 载体直接提供等价的流，不打开该 WebSocket。
